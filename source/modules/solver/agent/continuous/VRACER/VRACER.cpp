...
void VRACER::trainPolicy()
{
  ...
  _generationMiniBatchGenerationTime += std::chrono::duration_cast<std::chrono::nanoseconds>(endTime - beginTime).count();
  ...
  _generationStateSequenceTime += std::chrono::duration_cast<std::chrono::nanoseconds>(endTime - beginTime).count();
  ...
  if( _multiPolicyUpdate == "Together" )
  {
    ...
    _generationUpdateExperienceMetadataTime += std::chrono::duration_cast<std::chrono::nanoseconds>(endTime - beginTime).count();

    // Now calculating policy gradients
    beginTime = std::chrono::steady_clock::now();
    calculatePolicyGradients(miniBatch, policyInfo, 0);
    endTime = std::chrono::steady_clock::now(); 
    _generationCalculatePolicyGradientsTime += std::chrono::duration_cast<std::chrono::nanoseconds>(endTime - beginTime).count();
